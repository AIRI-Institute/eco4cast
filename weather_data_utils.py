...
def get_emission_data(
    ...
    """
    date = datetime.datetime(int(year_end), int(
        month_end), int(day_end)) + datetime.timedelta(days=1)
    year_end, month_end, day_end = str(
        date.year), str(date.month), str(date.day)
    if len(month_end) != 2:
        month_end = '0' + month_end
    if len(day_end) != 2:
        day_end = '0' + day_end
    response_string = f"https://api.energidataservice.dk/dataset/CO2Emis?offset=0&start="
...
def average_emission_data(
    ...
    df['Minutes5DK'] = pd.to_datetime(df['Minutes5UTC'])  # Опечатка?
...
def join_datasets(
    ...
    dataframe = pd.DataFrame(weather_dataframe.values,
                             columns=weather_dataframe.columns)
...
def get_multiple_historic_data(points,
                               year_start,
                               month_start,
                               day_start,
                               year_end,
        ...
        if 'weathercode' in weather_df.columns:
            if not include_weathercode:
                ...
                encoder = OneHotEncoder(
                    categories=[openmeteo_weathercodes], sparse_output=False,)
                onehot_weathercodes = encoder.fit_transform(
                    weather_df['weathercode'].values.reshape(-1, 1))
                weather_df = weather_df.drop(columns=['weathercode'])
                new_column_names = [f'weathercode_{i}' for i in range(
                    len(openmeteo_weathercodes))]
    ...
    return np.array(weather_matrices).transpose((1, 2, 0)), weather_df['time'], emission_df
...
def get_last_weather_data(latitude, longitude, lookback_days=14, parameters=parameters):
    ...
    dataframe = dataframe[dataframe['time'].dt.tz_localize(
        "UTC") < current_time]

    return dataframe


def get_multiple_last_weather_data(points,
                                   lookback_window,
                                   parameters=parameters,
                                   include_weathercode=False,
                                   co2_emission_delta_days=1):
    ...
    start_time = current_time - datetime.timedelta(days=delta_days,
                                                   hours=lookback_window)

    emission_df = None
    if co2_emission_delta_days is not None:
        emission_df = get_emission_data(
            year_start=start_time.year,
            month_start=start_time.month if start_time.month > 9 else f'0{start_time.month}',
            day_start=start_time.day if start_time.day > 9 else f'0{start_time.day}',
            year_end=current_time.year,
            month_end=current_time.month if current_time.month > 9 else f'0{current_time.month}',
            day_end=current_time.day +
            1 if current_time.day > 8 else f'0{current_time.day}',
        )
        emission_df = average_emission_data(emission_df)
    ...
    weather_data = np.array(weather_matrices).transpose(
        (1, 2, 0))[-lookback_window:]
...
if __name__ == '__main__':
    ...
    weather_points_dataset, datetimes, emission_df = get_multiple_historic_data(denmark_points,
                                                                                year_start=year_start,
                                                                                month_start=month_start,
                                                                                day_start=day_start,
                                                                                year_end=year_end,
