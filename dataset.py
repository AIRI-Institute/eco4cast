...
class WeatherCO2DataModule(LightningDataModule):
    def __init__(self, all_features_data, all_target_data, all_datetimes, lookback_window,
                 predict_window, num_workers, batch_size, split_sizes=[0.6, 0.2, 0.2]):
        super().__init__()
...
if __name__ == '__main__':
    ...
    dm = WeatherCO2DataModule(weather_points_dataset,
                              emission_data, datetimes, 96, 24, 10, 16)

    dm.setup('fit')
    dm.setup('test')

    print('Full data: ', len(emission_data))
    print('Total dataset samples: ', len(dm.train_dataset) +
          len(dm.val_dataset) + len(dm.test_dataset))
